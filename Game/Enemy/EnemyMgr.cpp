...
EnemyMgr::EnemyMgr()
{
	...
	for (auto& index : m_togekuri) {

		index = std::make_shared<Togekuri>();
...
void EnemyMgr::GenerateTogekuri(EnemyRoute::ROUTE_ID arg_routeID)
{
	...
	for (auto& index : m_togekuri) {
		...
		index->Generate(EnemyRoute::Instance()->GetRoute(arg_routeID), false);
...
int EnemyMgr::GetTargetTogekuriIndex(KazMath::Vec3<float> arg_playerPos, float arg_targetRange, float& arg_targetDistance)
{
	//生成している岩のなかからターゲットの範囲内のものを検索する。
	std::vector<int> inTargetIndex;
	std::vector<float> inTargetLength;
	...
	for (auto& index : m_togekuri) {
		...
		//インデックスを取得。
		inTargetIndex.emplace_back(static_cast<int>(&index - &m_togekuri[0]));
		inTargetLength.emplace_back(length);
