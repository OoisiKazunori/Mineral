#include "KazSoundManager.h"
#include"../Helper/KazHelper.h"

KazSoundManager::KazSoundManager()
{
...
void KazSoundManager::Finalize()
{
}

RESOURCE_HANDLE KazSoundManager::LoadSoundMem(std::string FILE_PASS, bool BGM_FLAG)
{
	//読み込み重複防止
...
void KazSoundManager::PlaySoundMem(RESOURCE_HANDLE HANDLE, int VOLUME, bool LOOP_FLAG)
{
...
void KazSoundManager::ChangeSoundMem(RESOURCE_HANDLE HANDLE, int VOLUME)
{
	float volumeTmp = VOLUME / 100.0f;
	masterSoundData[HANDLE].soundSorce->SetVolume(volumeTmp);
}

void KazSoundManager::StopSoundMem(RESOURCE_HANDLE HANDLE)
{
	if (KazHelper::IsitInAnArray(HANDLE, masterSoundData.size()))
	{
		if (masterSoundData[HANDLE].soundData != nullptr)
...
void KazSoundManager::ReleaseSoundMem(RESOURCE_HANDLE HANDLE)
{
	if (KazHelper::IsitInAnArray(HANDLE, masterSoundData.size()))
	{
		if (masterSoundData[HANDLE].soundData != nullptr)
